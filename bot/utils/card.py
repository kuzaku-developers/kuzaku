...
class RankCard:
    ...
    async def setStatus(self, status: discord.Status = None):
        if status is None:
            ...
            raise CardError('[Status] Используйте класс discord.Status!')
    ...
    async def setDisplayProcents(self, display: bool = None):
        if display is None:
            raise CardError('[DisplayProcents] Укажите True или False(False - изначально)!')
        elif display == True:
    ...
    async def create(self, path = None):
        ...
        def status():
            if self.status == 'idle':
                return Image.open(requests.get("https://discords.com/_next/image?url=https%3A%2F%2Fcdn.discordapp.com%2Femojis%2F763196801741881344.png%3Fv%3D1&w=64&q=75", stream = True).raw).convert('RGBA').resize((80, 80), Image.ANTIALIAS)
            elif self.status == 'online':
                return Image.open(requests.get("https://discords.com/_next/image?url=https%3A%2F%2Fcdn.discordapp.com%2Femojis%2F782348522681925672.png%3Fv%3D1&w=64&q=75", stream = True).raw).convert('RGBA').resize((80, 80), Image.ANTIALIAS)
            elif self.status == 'offline':
                return Image.open(requests.get("https://discords.com/_next/image?url=https%3A%2F%2Fcdn.discordapp.com%2Femojis%2F752991021180256266.png%3Fv%3D1&w=64&q=75", stream = True).raw).convert('RGBA').resize((80, 80), Image.ANTIALIAS)
            elif self.status == 'dnd':
                return Image.open(requests.get("https://discords.com/_next/image?url=https%3A%2F%2Fcdn.discordapp.com%2Femojis%2F782302404581785611.png%3Fv%3D1&w=64&q=75", stream = True).raw).convert('RGBA').resize((80, 80), Image.ANTIALIAS)
            else:
                return Image.open(requests.get("https://discords.com/_next/image?url=https%3A%2F%2Fcdn.discordapp.com%2Femojis%2F769033608924233768.png%3Fv%3D1&w=64&q=75", stream = True).raw).convert('RGBA').resize((80, 80), Image.ANTIALIAS)
        # | ------ | #
        # | Аватар | #
        ...
        if datexp() == 'Infinity':
            inf = Image.open(requests.get("https://discords.com/_next/image?url=https%3A%2F%2Fcdn.discordapp.com%2Femojis%2F798251908862705706.png%3Fv%3D1&w=64&q=75", stream = True).raw).convert('RGBA').resize((50, 45), Image.ANTIALIAS)
        ...
        idraw.text((xp_, 175), f"{self.xp}/{datexp()}", font = xpline, fill = self.textColor)
...
class WelcomeCard:
    ...
    async def create(self, path = None):
        ...
        image = Image.open(requests.get(self.abackground, stream = True).raw).convert('RGBA').resize((1270, 381), Image.ANTIALIAS)
        ...
        serveravatar = Image.open(requests.get(self.savatar, stream = True).raw).convert('RGBA')
...
class GoodbyeCard:
    ...
    async def create(self, path = None):
        ...
        savatar = Image.open(requests.get(self.savatar, stream = True).raw).convert('RGBA')
        ...
        disconnect = Image.open(requests.get("https://discords.com/_next/image?url=https%3A%2F%2Fcdn.discordapp.com%2Femojis%2F284499766145056768.png%3Fv%3D1&w=64&q=75", stream = True).raw).convert('RGBA').resize((150, 150), Image.ANTIALIAS)
...
class DemCard:
    ...
    async def setText(self, text: str = None):
        if text is None:
            raise CardError('[Dem-Text] Укажите текст. Или __help__ для получения помощи!')
        elif text == "__help__":
            print("""
            [Dem-Text] Помощь:
    ...
    async def create(self, path = None):
        ...
        image = Image.open(requests.get(self.img, stream = True).raw).convert('RGBA').resize((1080, 630), Image.ANTIALIAS)
        ...
        idraw.text(((1310 - size_1[0]) / 2, 775), self.text1, font = line, fill = self.textColor)
        size_2 = idraw.textsize(self.text2, font = line2)
        idraw.text(((1310 - size_2[0]) / 2, 880), self.text2, font = line2, fill = self.textColor)
