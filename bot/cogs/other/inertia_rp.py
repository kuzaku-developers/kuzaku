...
class roleplay(commands.Cog):
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            ...
            Option('количество', 'укажи количество валюты для перевода', Type.INTEGER, required=True),
    ...
    async def pay(self, ctx):
        ...
        try:
            if dbrpgetuser(ctx.user.id)['rubles'] < money:
                await msg.edit(
                    embed=discord.Embed(title='ошибка', description='у тебя нет столько денег! перевожу все!'))
                money2 = dbrpgetuser(ctx.user.id)['rubles']
                await dbrpaddrubles(ctx.user.id, -money2)
                await dbrpaddrubles(user.id, money2)

                await msg.edit(embed=discord.Embed(title='перевод завершен',
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        if payload.emoji.name == "🛬" and payload.channel_id == 837345395667501118:
            ...
            Role = discord.utils.get(payload.member.guild.roles, id=837312438881353748)
            await payload.member.add_roles(Role)
            await Channel.send(embed=discord.Embed(title='прилет', description=f'{payload.member.mention} прилетел!'),
                               delete_after=5)
            # -----end of reg----#
        # -----start of passport----#
        if payload.emoji.name == "📝" and payload.channel_id == 837312495634481202:
            ...
            await payload.member.send(
                embed=discord.Embed(title='регистрация', description=f'{payload.member.mention}, регистрируйся!'),
                delete_after=5)
            await payload.member.send(
            ...
            try:
                regname = await self.bot.wait_for('message', check=check(payload.member), timeout=30)
            ...
            if not len(f'{regname} | {dbgetrpid(payload.member.id)}') > 32:
                ...
                await payload.member.send('слишком длинный ник! твой ник будет твоим дискорд ником!')
                await payload.member.edit(nick=f'{payload.member.name} | {dbgetrpid(payload.member.id)}')
            await payload.member.send(
                embed=discord.Embed(title="регистрация", description='ты прошел регистрацию! приятного рп!'),
                delete_after=20)
            rolee = payload.member.guild.get_role(837312438881353748)
            await payload.member.remove_roles(rolee)
            Role = discord.utils.get(payload.member.guild.roles, id=837340231937818636)
            await payload.member.add_roles(Role)
            docschannel = self.bot.get_channel(837312548856135720)
            await docschannel.send(embed=discord.Embed(title='регистрация!', description=f'''
...
номер человека: {dbgetrpid(payload.member.id)}
        ...
        if payload.emoji.name == "🙏" and payload.channel_id == 837340041176416277:
            try:
                if int(dbrpgetuser(payload.member.id)['rubles']) >= 150 and dbrpgetuser(payload.member.id)[
                    'home'] == True or int(dbrpgetuser(payload.member.id)['rubles']) >= 600:
                    Rolebomz = discord.utils.get(payload.member.guild.roles, id=835462674879741962)
                    Roleman = discord.utils.get(payload.member.guild.roles, id=835539034914029618)
                ...
                if int(dbrpgetuser(payload.member.id)['rubles']) >= 500 and dbrpgetuser(payload.member.id)[
                    'home'] == True or int(dbrpgetuser(payload.member.id)['rubles']) >= 600:
                    Rolebomz = discord.utils.get(payload.member.guild.roles, id=835462674879741962)
                    Roleman = discord.utils.get(payload.member.guild.roles, id=835539034914029618)
                    await payload.member.remove_roles(Rolebomz)
        ...
        if str(payload.emoji.name) == '1️⃣' and payload.channel_id == 837663284207157339:
            ...
            airrole = discord.utils.get(payload.member.guild.roles, id=837312389102698516)
            if airrole in payload.member.roles:
                await payload.member.send(embed=discord.Embed(title='у тебя уже есть airpods++',
                                                              description=f'ты не можешь купить те же наушники!'))
            elif int(dbrpgetuser(payload.user_id)['rubles']) >= (45000 + getnalog(45000)):
                dbrpaddrubles(payload.user_id, -(45000 + getnalog(45000)))
                await payload.member.add_roles(airrole)
                await payload.member.send(embed=discord.Embed(title='ты купил airpods++',
                ...
                await payload.member.send(embed=discord.Embed(title='ты не смог купить airpods++',
        ...
        if payload.emoji.name == "2️⃣" and payload.channel_id == 837663284207157339:
            macrole = discord.utils.get(payload.member.guild.roles, id=837312392118927370)
            await msg.add_reaction("2️⃣")
            await msg.remove_reaction("2️⃣", payload.member)

            if macrole in payload.member.roles:
                await payload.member.send(embed=discord.Embed(title='у тебя уже есть mac pro',
                ...
                await payload.member.send(embed=discord.Embed(title='ты купил mac pro',
                ...
                await payload.member.send(embed=discord.Embed(title='ты не смог купить mac pro',
        ...
        if payload.emoji.name == "3️⃣" and payload.channel_id == 837663284207157339:
            ...
            msg = await self.bot.get_channel(payload.channel_id).fetch_message(payload.message_id)
            macbookrole = discord.utils.get(payload.member.guild.roles, id=837312387375562832)
            await msg.add_reaction('3️⃣')
            await msg.remove_reaction('3️⃣', payload.member)
            try:
                if macbookrole in payload.member.roles:
                    await payload.member.send(embed=discord.Embed(title='у тебя уже есть macbook!',
                                                                  description=f'ты не можешь купить macbook еще раз!'))
                elif dbrpgetuser(payload.user_id)['rubles'] >= 120000 + getnalog(120000):
                    dbrpaddrubles(payload.user_id, -120000 + getnalog(120000))

                    await payload.member.send(embed=discord.Embed(title='ты купил macbook',
                    ...
                    await payload.member.send(embed=discord.Embed(title='ты не смог купить macbook',
        ...
        if payload.emoji.name == "4️⃣" and payload.channel_id == 837663284207157339:
            msg = await self.bot.get_channel(payload.channel_id).fetch_message(payload.message_id)
            samsungrole = discord.utils.get(payload.member.guild.roles, id=837312380894969888)
            await msg.add_reaction('4️⃣')
            await msg.remove_reaction('4️⃣', payload.member)
            try:
                if samsungrole in payload.member.roles:
                    await payload.member.send(embed=discord.Embed(title='у тебя уже есть Samsung S10+!',
                    ...
                    await payload.member.send(embed=discord.Embed(title='ты купил Samsung S10+',
                    ...
                    await payload.member.send(embed=discord.Embed(title='ты не смог купить Samsung S10+',
        ...
        if payload.emoji.name == "5️⃣" and payload.channel_id == 837663284207157339:
            msg = await self.bot.get_channel(payload.channel_id).fetch_message(payload.message_id)
            i6role = discord.utils.get(payload.member.guild.roles, id=837312385052180490)
            await msg.add_reaction('5️⃣')
            await msg.remove_reaction('5️⃣', payload.member)
            try:
                if i6role in payload.member.roles:
                    await payload.member.send(embed=discord.Embed(title='у тебя уже есть iphone 6s!',
                    ...
                    await payload.member.send(embed=discord.Embed(title='ты купил iphone 6s',
                    ...
                    await payload.member.send(embed=discord.Embed(title='ты не смог купить iphone 6s',
        ...
        if payload.emoji.name == "🧑‍🚒" and payload.channel_id == 835791464109572096:
            ...
            if dbrpgetuser(payload.user_id)['rubles'] >= 10:
                ...
                Firerole = discord.utils.get(payload.member.guild.roles, id=835932798753046578)
                await payload.member.add_roles(Firerole)
                await payload.member.send(embed=discord.Embed(title='ты стал пожарным!',
                ...
                message1 = await payload.member.send(embed=discord.Embed(title='ты не смог устроиться на работу',
                ...
                try:
                    ...
                    messagee = await payload.member.send(embed=discord.Embed(title='уговариваем работодателя',
                                                                             description=f'пробуем устроить тебя на работу без денег'),
                                                         delete_after=31)
                    await asyncio.sleep(3)
                    if random.randint(1, 100) <= chance:
                        await messagee.edit(
                            embed=discord.Embed(title='получилось!',
                                                description=f'ты смог устроиться! поздравляю! '),
                            delete_after=31)
                        dbrpsetjson(payload.user_id, 'firefighter', True)
                    else:
                        await messagee.edit(
                            embed=discord.Embed(title='не получилось!',
                                                description=f'ты не смог устроиться! увы :( '),
                            delete_after=31)

                except:
                    await payload.member.send('реакции не было, отменяю!', delete_after=10)
            # ---end of works--#
        # ---start of dark.NET--#
        ...
        if payload.emoji == hardcoin and payload.channel_id == 837746401520779334:
            ...
            try:
                ...
                coins = dbrpgetuser(payload.user_id)['darkcoins']
            await payload.member.send(embed=discord.Embed(title='твоя статистика в dark.NET', description=f'''
        ...
        if payload.emoji.name == '❓' and payload.channel_id == 837394878866260019:
            ...
            await payload.member.send(embed=discord.Embed(title='статистика', description=f'''
Паспорт:
```
...
'''), delete_after=30)
        ...
        if payload.emoji.name == '🤑' and payload.channel_id == 838829237770125342:
            ...
            try:
                moneyhave = await self.bot.wait_for('message', check=check(payload.member), timeout=30)
            ...
            try:
                ...
                if int(dbrpgetuser('federation')['rubles']) < int(moneyhave):
                    await payload.member.send('в казне нет столько денег! забираю всю казну...')

                    dbrpaddrubles(payload.user_id, int(dbrpgetuser('federation')['rubles']))
                    await payload.member.send(f'готово! ты взял {dbrpgetuser("federation")["rubles"]} рублей!')
                    dbrpaddrubles('federation', -int(dbrpgetuser('federation')['rubles']))
                else:
        ...
        if payload.emoji.name == '💰':
            ...
            try:
                ...
                if int(dbrpgetuser(payload.user_id)['rubles']) < int(moneyhave):
                    await payload.member.send('у тебя нет столько денег! забираю все...')

                    dbrpaddrubles('federation', int(dbrpgetuser(payload.user_id)['rubles']))
                    await payload.member.send(f'готово! ты положил {moneyhave} рублей!')
                    dbrpaddrubles(payload.user_id, -int(dbrpgetuser(payload.user_id)['rubles']))
                else:
                    dbrpaddrubles('federation', int(moneyhave))
                    dbrpaddrubles(payload.user_id, -int(moneyhave))
                    await payload.member.send(f'готово! ты положил {moneyhave} рублей!')
                    ...
                    if firework[i]["health"] >= 1:
                        ...
                        if dbrpgetuser(i)['health'] <= 20:
                            if firework[i]['health'] <= 0:
                                ...
                                await user.send(
                                embed=discord.Embed(title='здоровье',
